...
try {
    ...
    // Reject request
    $enrollmentModel->rejectEnrollmentRequest($requestId);
    ...
    EmailService::notifyEnrollmentRejected($request['student_id'], $teacherName, $request['course_name']);

    sendSuccess('Enrollment request rejected successfully');

} catch (Exception $e) {
    sendError('Failed to reject enrollment request: ' . $e->getMessage(), 500);
}
